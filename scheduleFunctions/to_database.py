        ...
        for requirement_label in data[major_label].keys():
            try:
                requirement = Requirement.objects.get(
                    major_label=major_label, requirement_label=requirement_label
                )
            ...
            for course in courses:
                ...
                try:
                    selected_course = Course.objects.get(
                        subject=subject, class_number=course_number
                    )
...
def store_plan(json_file) -> None:
    ...
    for course, course_details in data.items():
        ...
        for other_course in course_details["equivalent_courses"]:
            try:
                selected_other_course = Course.objects.get(
                    subject=other_course[:-4], class_number=int(other_course[-4:])
                )
            except Course.DoesNotExist:
        ...
        try:
            selected_semester = PlanSemester.objects.get(
                year=year_keys[year], semester=semester_keys[semester]
            )
...
def store_schedule(json_file: str) -> None:
    ...
    for subject, courses in schedule.items():
        for course, details in courses.items():
            try:
                selected_course = Course.objects.get(
                    subject=subject, class_number=int(course)
                )
            except Course.DoesNotExist:
                selected_course = Course(
                    subject=subject,
            ...
            credit_hours = details["sections"][next(iter(details["sections"]))][
                "Credit Hours"
            ]
            try:
            ...
            for section, section_details in details["sections"].items():
                if (
                    section_details["Location"]
                    in ["Totally Online", "To Be Announced", "Partially Online"]
                    or section_details["Time"] == "TBA"
                ):
                ...
                try:
                    selected_section = Section.objects.get(
                        course=selected_course, section_number=int(section)
                    )
                ...
                selected_section.section_id = int(
                    section_details["Class Number"].split(" ")[0]
                )

                try:
                    selected_professor = Proffessor.objects.get(
                        name=section_details["Instructor"]
                    )
                except Proffessor.DoesNotExist:
                    selected_professor = Proffessor(name=section_details["Instructor"])
                    selected_professor.save()

                selected_section.professor = selected_professor
                room_dict = re.search(
                    "(?P<building>.+)\s?(?P<room_number>[0-9]*)",
                    section_details["Location"],
                )
                ...
                try:
                    selected_room = Room.objects.get(
                        building=room_dict.group("building"), room_number=room_number
                    )
                ...
                selected_room.capacity = max(
                    selected_room.capacity, int(section_details["Class Max"])
                )
                selected_room.save()
                ...
                selected_days = Day.objects.filter(
                    day_of_week__in=[
                        Day.DAY_OF_WEEK_CHOICES[day]
                ...
                try:
                    selected_time = (
                        TimeSlot.objects.annotate(num_days=Count("days"))
                        .filter(days__in=selected_days)
                        .filter(num_days=len(selected_days))
                    ...
                    selected_time = TimeSlot(
                        start_time=start_time, end_time=end_time, credits=credit_hours
                    )
                    selected_time.save()
...
def store_schedule_changes(
    section_id: int, start_time: date, end_time: date, days: str
) -> None:
    ...
    try:
        ...
        selected_days = Day.objects.filter(
            day_of_week__in=[Day.DAY_OF_WEEK_CHOICES[day] for day in days]
        )
        print(f"Original days: {selected_section.time_slot.days.all()}")
        print(f"Original start_time: {selected_section.time_slot.start_time}")
        print(f"New selected days: {selected_days}")
        ...
        # print(updated_time)
        print(
            f"Something went wrong, timeslot doesn't exist: {(start_time, end_time, days)}"
        )
